...
from login.models import User, DepartmentPollData
...
def calResult(request):
    ...
    if user.is_poll_1 == 'True':
        poll_result = DepartmentPollData.objects.filter(user__username=infoDict['username']).first()
        if not poll_result:
        ...
        poll_result = DepartmentPollData(user=user, group=user.group, question1=questions[1], question2=questions[2], question3=questions[3], question4=questions[4], question5=questions[5])
        user.is_poll_1 = 'True'
        request.session['is_poll_1'] = 'True'
        poll_result.save()
        user.save()
        if questions[1] == "不合格" and questions[2] == "不合格" and questions[3] == "不合格" and questions[4] == "不合格" and questions[5] == "不合格":
            return redirect(reverse('departmentPoll:poll_reason'))
        infoDict['question1'] = poll_result.question1
        infoDict['question2'] = poll_result.question2
        infoDict['question3'] = poll_result.question3
        infoDict['question4'] = poll_result.question4
        ...
        if request.method == 'POST':
            ...
            if pollReason:
                ...
                if poll_result:
                    ...
                    poll_result.save()
                    return redirect(reverse('departmentPoll:poll_index'))
                else:
                    return redirect(reverse('departmentPoll:poll_index'))
            else:
...
def adminResultView(request):
    ...
    if request.method == 'POST':
        ...
        queryResultSet = DepartmentPollData.objects.filter(group__groupName=target_group)
